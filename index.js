...
mainDialog.onDefault((session, args) => {
    ...
    Promise.all([rp(urls[0]),rp(urls[1])]).then((values) => {
        ...
        const message = tsmlj`
            *calamarcopolo:* ${args.intents[0].intent}, ${args.intents[0].score};
            *roboviário:* ${roboviarioResult.intents[0].intent}, ${roboviarioResult.intents[0].score};.
            *cortana:* ${cortanaResult.intents[0].intent}, ${cortanaResult.intents[0].score};
        `;
